...
### Noise-level sweep (H1)

5 noise levels × 3 seeds × {Bellman, Q-Learning}, 1500 episodes per cell.
The companion to the alpha-decay sweep — the *other* OAT dimension.
Generated by `analysis/noise_sweep.py`; chart at
`results/analysis/noise_sweep.png` (line chart with SEM error bars).
Directly probes H1's "noise breaks Bellman" prediction by varying noise
explicitly while everything else is held constant.

### Per-seed convergence scatter

Generated inline by [the research notebook](notebooks/research_analysis.ipynb)
(§6); chart at `results/analysis/convergence_scatter.png`. A standalone
**scatter plot** of per-seed `episodes-to-half-target` (x) vs
`last-50 mean reward` (y), one colour per algorithm — the §20.5.b
correlation-style scatter the alpha-decay sweep's bar chart and the
multi-seed jitter overlay don't directly cover.

### Q-table heatmap

Also generated by the research notebook; chart at
`results/analysis/q_table_heatmap.png`. A standalone **heatmap** of
`max Q(s)` per cell after 1500 Q-Learning episodes on the medium
board — the §20.5.b heatmap chart-type evidence in artefact form (not
just GUI overlay).
...
| **Pytest + coverage** | pre-push, CI | 331 tests pass, ≥85% line coverage (current: 97.17 %). Coverage gate is in `pyproject.toml`'s `addopts`, so any plain `uv run pytest` enforces it. |
...
| **Test coverage** | ≥ 85 % | `pytest --cov` | Current: **97.17 %**, gate via `--cov-fail-under=85` in `addopts` |
...
```bash
uv run pytest tests/                              # 331 tests, 97.17 % coverage, gate enforced
